...
class SignalpTWeight(Module):
    def __init__(self, year, branch = "GenZll"):
        self.year = year
        self.branch = branch

        #Signal spectrum correction file, where s4 = 4th and current signal iteration
        file   = os.environ['CMSSW_BASE'] + "/src/PhysicsTools/NanoAODTools/data/zpt/signal_z_correction_s4_" + year + ".root"

        self.file  = ROOT.TFile(file , "READ")
        if self.file.IsZombie():
            raise Exception("Signal pT correction file not found!")
        name = 'hratio'
        if not self.file.GetListOfKeys().Contains(name):
            raise Exception("Signal pT correction histogram not found!")
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):        
        ...
        self.out.branch("SignalpTWeight",'F')
    ...
    def analyze(self, event):
        ...
        if not hasattr(event, "GenJet_pt"): #Data/Embedding
            self.out.fillBranch("SignalpTWeight", 1.)
            return True
        ...
        self.out.fillBranch("SignalpTWeight"    , weight)
        return True
