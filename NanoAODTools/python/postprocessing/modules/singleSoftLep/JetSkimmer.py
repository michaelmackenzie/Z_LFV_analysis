...
#        self.out.branch("nBJet",'F')        
 ...
 #         print self.Selection[0]
        ...
        bjets={"pt":[],"eta":[],"phi":[],"btagDeepB":[],"btagDeepC":[]}
...
#        nBJet=0; nCJet=0;
        ...
        if len(jets)<self.nGoodJetMin[0]:
           print len(jets),"good jets found with lower limit",self.nGoodJetMin, "skip"
           return False

        if len(bjets)>self.nBJetMax[0]:
            print len(bjets),"good b-jets found with upper limit",self.nBJetMax, "skip"
            return False
